...
public class FileUploadController {
    ...
    @PostMapping("/upload/vedio")
    public ResponseEntity<Map<String,String>> uploadVedio (@RequestParam("file")MultipartFile file){
    ...
    @GetMapping("/vedio/{uuid}")
    public ResponseEntity<Resource> serveVideo(
            @PathVariable String uuid,
            @RequestHeader(value = "Range",required = false) String rangeHeader,
            @RequestHeader(value = "token",required = false) String tokenPara
    ){
        return fileUploadService.serveVedio(uuid,rangeHeader);
    }
